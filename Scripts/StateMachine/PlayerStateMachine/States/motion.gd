...
const PLAYER_MOVEMENT_STATS = preload("res://Resources/player_movement_stats.tres")
...
func replenish_sprint(delta: float, replinishment_rate: float = 1.0) -> void:
	sprint_remaining = min(sprint_remaining + (delta * replinishment_rate), PLAYER_MOVEMENT_STATS.sprint_duration)
